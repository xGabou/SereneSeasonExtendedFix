...
public class DefaultSnowEnvironmentHandler implements ISnowEnvironmentHandler {
    ...
    public int getBlocksToReplace(ServerLevel level, BlockPos pos) {
        ...
        float temperature = SeasonHooks.getBiomeTemperature(level, level.getBiome(pos), pos, level.getSeaLevel());
        boolean cold = SeasonHooks.coldEnoughToSnowSeasonal(level, pos, level.getSeaLevel());
    ...
    public boolean isColdEnoughForSnow(ServerLevel level, BlockPos pos) {
        return isOverworld(level) && SeasonHooks.coldEnoughToSnowSeasonal(level, pos,level.getSeaLevel());
    }
